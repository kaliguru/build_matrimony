...
const getCommunity = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (authorizationResult.statusCode == 200) {
            ...
            let countSql = "SELECT COUNT(*) as totalCount  FROM community";
            if (req.body.name) {
            ...
            let sql = `SELECT * FROM community WHERE isDelete = 0 `;
            if (req.body.name) {
        ...
        if (validationResult && validationResult.statusCode == 200) {
            ...
            if (authorizationResult.statusCode == 200) {
                ...
                if (checkResult && checkResult.length > 0) {
                    ...
                    if (req.body.id) {
                        let sql = `UPDATE community SET name = '` + req.body.name + `' where id = ` + req.body.id + ``;
                        let result = yield query(sql);
                        ...
                        let sql = `INSERT INTO community(name, createdBy, modifiedBy) VALUES('` + req.body.name + `',` + userId + `,` + userId + `);`;
                        let result = yield query(sql);
                        if (result && result.affectedRows > 0) {
